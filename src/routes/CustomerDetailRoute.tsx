...
const customerRoute = getRouteApi('/customers/$id');
...
export function CustomerDetailRoute() {
  ...
  const activeCustomer = customers.find((customer) => customer.id === id);
  ...
  const handleDeleteConfirm = () => {
    ...
    toastManager.add({
      ...
      description: activeCustomer ? `"${activeCustomer.name}" was deleted.` : undefined,
      timeout: 4000,
    });
  };

  if (!activeCustomer) {
    return <div className="flex flex-1 items-center justify-center text-sm text-slate-500">Customer not found.</div>;
  }

  return (
    <MasterDetailShell
      masterListHeading="Customer Workspace List"
      detailKey={activeCustomer.id}
      contextRailHeading="Contextual Tool Actions"
      masterList={
        <>
          <header className="flex h-14 items-center gap-3 border-b border-slate-200 px-4 bg-slate-50">
            <search className="flex-1">
              <form onSubmit={(e) => e.preventDefault()} className="relative">
                ...
                <input
                  ref={searchInputRef}
                  type="search"
                  placeholder="Filter customers... (⌘K)"
                  value={searchQuery}
          ...
          <nav aria-label="Select Customer" className="flex-1 overflow-y-auto p-3" ref={listScrollRef}>
            {filteredCustomers.length > 0 ? (
              <ul className="relative" style={{ height: virtualizer.getTotalSize() }}>
                {virtualizer.getVirtualItems().map((virtualRow) => {
                  const customer = filteredCustomers[virtualRow.index];
                  const isActive = customer.id === id;
                  return (
                    <li
                      key={customer.id}
                    ...
                    >
                      ...
                      >
                        <div className="flex w-full items-center justify-between gap-2">
                          <h3 className="font-semibold text-sm text-slate-900 group-aria-[current=page]:text-blue-900">{customer.name}</h3>
                        ...
                        <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed">{customer.company}</p>
                        <time className="mt-1 text-xxs text-slate-400" dateTime={customer.joinedDate}>{customer.joinedDate}</time>
                      </Link>
                    </li>
                  );
                })}
              ...
              <div className="p-8 text-center text-sm text-slate-500">No customers match criteria.</div>
            )}
          </nav>
          ...
          <header className="border-b border-slate-200 pb-6 mb-6">
            <nav aria-label="Breadcrumb" className="mb-2">
              <ol className="flex items-center gap-2 text-xs text-slate-400 font-mono uppercase tracking-wider">
                <li>
                  <Link to="/customers" className="hover:text-slate-600 hover:underline">Customers</Link>
                </li>
                <li aria-hidden="true">/</li>
                <li aria-current="page">{activeCustomer.id}</li>
              </ol>
            </nav>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 mb-2">{activeCustomer.name}</h1>
            <p className="text-sm text-slate-500">
              Company: <span className="font-semibold text-slate-700">{activeCustomer.company}</span> • Customer since <time dateTime={activeCustomer.joinedDate}>{activeCustomer.joinedDate}</time>
            </p>
          </header>
        ...
        <>
              ...
              <Dialog.Portal>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                  <Dialog.Popup
                    ...
                    render={<section aria-labelledby="delete-customer-dialog-title" />}
                  >
                    <header className="mb-4">
                      <Dialog.Title id="delete-customer-dialog-title" className="text-base font-bold text-slate-900">
                        Confirm Deletion
                      </Dialog.Title>
                      <Dialog.Description className="mt-1 text-xs text-slate-500">
                        Are you sure you want to delete <span className="font-semibold text-slate-700">"{activeCustomer.name}"</span>? This action is permanent and cannot be undone.
                      </Dialog.Description>
                    </header>
          ...
          <section className="space-y-2">
            ...
            <ul className="space-y-1.5">
              <li>
                <a href={activeCustomer.websiteUrl} className="group flex items-center justify-between rounded-md p-1.5 text-xs text-slate-600 hover:bg-slate-200/50 hover:text-slate-900 transition-colors">
                  <span className="flex items-center gap-2 truncate"><Globe className="h-3.5 w-3.5 shrink-0 text-slate-400" /> Company Website</span>
